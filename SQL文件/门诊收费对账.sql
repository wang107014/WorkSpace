...
//His交易明细表
SELECT * FROM DHC_BillExtTradePay WHERE ETP_PayMode=4 AND ETP_HisPrtStr IN (405999,406000)

SELECT ETC_HISPRT_DR, * from DHC_BillExtTradeConSub WHERE ETC_ETP_ParRef=21307

SELECT * FROM DHC_BillExtTradePay WHERE ETP_PayMode=4 AND ETP_TradeChannel="YHK"
SELECT * FROM DHC_BillExtTradePay WHERE ETP_PayMode=4 AND ETP_Papmi_Dr IN ('15239')

//第三方交易明细表
SELECT * FROM DHC_BillExtTradeBalance WHERE ETB_OutTradeNo IN ('215618260977')
