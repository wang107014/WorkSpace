...
13、门诊挂号导诊单
	...
	w ##class(web.DHCOPAdmReg).OPRegistBroker()
	w ##class(web.DHCOPAdmReg).GetPrintData(50903,"","N")
...
7、在医嘱列表显示医嘱信息
	...
	d ##class(%ResultSet).RunQuery("web.DHCOPAdmFind","GetADMOrder","10901","1","^^","5","119")
...
4、退费凭条
	...
	w ##class(web.udhcOPRefEdit1).GetRefundInfoNew("2251||1^2251||2^2251||3","381877")
...
###2、门诊收据查询
	在udhcOPINV.js中    
	d ##class(%ResultSet).RunQuery("web.udhcOPQUERY","INVQUERY11","ALL","","","","",64824,64824,"","ALL","","","","","","","")
...
###3、预交金退款
	...
	1、补交易
		UDHCAccRefundDeposit.js中的RePosPay_Click
...
##医保大额门诊集中报销
	...
	insuourhospitalpay.js

	d ##class(%ResultSet).RunQuery("web.DHCINSUDivQue","FindINVPRTForINSUOPDivide")