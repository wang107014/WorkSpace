...
###5、输入登记号
	...
	UDHCJFIPReg.js中的getpatinfo方法中

	有效住院证数量
		w ##class(web.DHCDocIPBookingCtl).GetIPBKNum("94","")
	数量大于2弹出选择框
		dhcdocipbookqry.csp
		住院证记录列表 双击触发事件：DHCDocIPBookList.js
	...
	获取住院证信息
		w ##class(web.UDHCJFIPReg).getpatinfo("4","","","121")
...
##8、医保结算
	...
	医保预结算
	在dhcbill/dhcipbill/dhcipbillchargecontrol.js中的insuPreChargeClick
    DHCInsuPort.js中的InsuIPDividePre  --住院预结算

	医保结算
	在dhcbill/dhcipbill/dhcipbillchargecontrol.js中的insuChargeBtn
	DHCInsuPort.js中的InsuIPDivide --住院预结算
