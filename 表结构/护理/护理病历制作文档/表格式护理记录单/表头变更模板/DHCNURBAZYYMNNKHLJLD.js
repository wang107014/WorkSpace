...
if (GetNurseRecSet){
	var ret=cspRunServerMethod(GetNurseRecSet.value,"DHCNURBAZYYMNNKHLJLD");
	var hh=ret.split("^");
 ...
 function btlist()
{
		...
		var lnk= "dhcnuremrcomm.csp?"+"&EmrCode=DHCNURHEADCHANGE&EpisodeID="+EpisodeID //""+"&Status="+""  ;//"&DtId="+DtId+"&ExamId="+ExamId
	 
     var ddd=kkk%2 
	  
	   if (ddd==0)
	   {	   	
	   wind3= window.open(lnk,"htm",'left=10,toolbar=no,location=no,directories=no,resizable=yes,width=1370,height=500');
	   kkk=kkk+1;
	   }
	   else 
	   	{
...
//debugger;
  var GetCurrHeadDR=document.getElementById('GetCurrHeadDR');
  //alert(EpisodeID+"@"+session['LOGON.USERID']+"@"+EmrCode)
	var ret=cspRunServerMethod(GetCurrHeadDR.value,EpisodeID,session['LOGON.USERID'],EmrCode);
...
function find2(){
  ...
  // alert(parent.frames.length);
	...
	var parr = adm + "^" + StDate.value + "^" + StTime.value + "^" + Enddate.value + "^" + EndTime.value + "^"+"DHCNURBAZYYMNNKHLJLD" + "^" + IfCancelRec+"^"+CurrHeadDr;
	// debugger;
	//alert(parr)
...
function PrintSet()
{
 ...
 //debugger;
   ...
   // debugger;
    REC=new Array();
 	var a=cspRunServerMethod(GetNurPrnSet.value,"DHCNURBAZYYMNNKHLJLD",EpisodeID,'addprnset');
    mygrid.store.loadData(REC);   
...
{  //查询出入液量合计
	...
	var mygrid=Ext.getCmp("mygrid");
  //alert(adm+"|"+StDate.value+"|"+StTime.value+"|"+Enddate.value+"|"+EndTime.value+"|"+"DHCNURBAZYYMNNKHLJLD")
 	var a=cspRunServerMethod(GeInOutAmount.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURBAZYYMNNKHLJLD");
...
{  //查询出入液量合计
	...
	var mygrid=Ext.getCmp("mygrid");
  //alert(adm+"|"+StDate.value+"|"+StTime.value+"|"+Enddate.value+"|"+EndTime.value+"|"+"DHCNURBAZYYMNNKHLJLD")
 	var a=cspRunServerMethod(GeInOutAmountNL.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURBAZYYMNNKHLJLD");
 	//alert(a)
	if (a=="") { Ext.Msg.alert('提示', "无护理记录数据!"); return; }
	//additm();
	additmstat(Enddate.value,EndTime.value);
	...
	for (var i = 0; i < list.length; i++) {
	  ...
	  var flag="0";
		...
		if ((str!="")&&(flag=="0"))
		{
			...
			var a=cspRunServerMethod(RecSave.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBAZYYMNNKHLJLD",session['LOGON.GROUPDESC']);
			if (a!="0")
			{
				alert(a);
				return;
...
function MultiFun()
{
	...
	var ret= cspRunServerMethod(GetMulti.value, "DHCNURBAZYYMNNKHLJLD");
	...
	var tabstr=cspRunServerMethod(getcheckform.value, "DHCNURBAZYYMNNKHLJLD",ab);
...
function multiSel(ret)
{
    ...
    var itname=grid1.getSelectionModel().getSelections()[0].get("itm4"); 
	  var getcheckform=document.getElementById('getcheckform');
		//alert(itname+"!!"+code)
		var ret=cspRunServerMethod(getcheckform.value, "DHCNURBAZYYMNNKHLJLD",itname,code,"");	
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		str=str+"^DiagnosDr|"+DiagnosDr+"^HeadDR|"+CurrHeadDr;
		//alert(str);
		var a=cspRunServerMethod(SaveOutInNL.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBAZYYMNNKHLJLD");
		find();
	}
}
function InOutSumNL()
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		str=str+"^DiagnosDr|"+DiagnosDr+"^HeadDR|"+CurrHeadDr;
		//alert(str);
		var a=cspRunServerMethod(SaveOutIn.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBAZYYMNNKHLJLD");
		find();
	}
}
function InOutSum()
...
function Measure()
{
   ...
   // debugger;
	...
	var parr="DHCNURBAZYYMNNKHLJLD^"+EpisodeID+"^CaseMeasureXml^"+rowid;
	//alert(parr);
	var arr = new Array();
	var a = cspRunServerMethod(pdata1, "", "DHCPatOrdList", EpisodeID, "");
	arr = eval(a);
...
function sureMeasure()
{
   ...
   // var parr="DHCNURBAZYYMNNKHLJLD^"+EpisodeID+"^CaseMeasureXml";
...
function printNurRec()
{
		...
		//alert("ddd");
		//debugger;
		var a=cspRunServerMethod(GetPrnSet.value,"DHCNURBAZYYMNNKHLJLD",EpisodeID); //page, caredattim, prnpos, adm,Typ,user
		if (a=="") return;
		var GetLableRec=document.getElementById('GetLableRec');
		var LabHead=cspRunServerMethod(GetLableRec.value,"DHCNURBAZYYMNNKHLJLD^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID']);
		//alert(LabHead)
		//alert("DHCNURBAZYYMNNKHLJLD^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID'])
		/*if(hh1[1]="张海强")
		{
		...
		PrintComm.ItmName = "DHCNURMouldPrn_ZYYMNNK"; //338155!2010-07-13!0:00!!
		//debugger;
	//	var parr="parr:"+EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURBAZYYMNNKHLJLD!"+CurrHeadDr+"^";
	var sttym=Ext.getCmp("sttym").getValue();
	  ...
	  var parr="parr:"+EpisodeID+"!"+StDate+"!"+StTime2+"!"+Enddate+"!"+EndTime2+"!DHCNURBAZYYMNNKHLJLD!"+CurrHeadDr+"^";  //DHCNURBATYSJW界面模板名，相应修改
		//var parr=EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURBAZYYMNNKHLJLD!"+CurrHeadDr+"^";
		PrintComm.ID = "";
		PrintComm.MultID = "";
		...
		//alert(pages+","+CareDateTim+","+stprintpos+","+EpisodeID+","+"DHCNURBAZYYMNNKHLJLD"+","+session['LOGON.USERID']+","+PrintComm.PrnFlag);
		//PrnFlag==1说明是打印预览
		if (PrintComm.PrnFlag==1) return;
		//如果原记录保存打印到第8页则当打印第8页之前页时不保存打印记录
		if (pages<aa[0]) return;
		var a=cspRunServerMethod(SavePrnSet.value,pages,CareDateTim,stprintpos,EpisodeID,"DHCNURBAZYYMNNKHLJLD",session['LOGON.USERID']); //page, caredattim, prnpos, adm,Typ,user
		//find();
}
function printNurRecall()
{
		...
		//alert(PrintComm.Pages);
		PrintComm.SetConnectStr(CacheDB);
		PrintComm.ItmName = "DHCNURMouldPrn_ZYYMNNK"; //338155!2010-07-13!0:00!!   //打印模板名
		//debugger;
		var parr="parr:"+EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURBAZYYMNNKHLJLD!"+CurrHeadDr+"^";   //DHCNURBATYSJW界面模板名，相应修改
		//var parr=EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURBAZYYMNNKHLJLD!"+CurrHeadDr;
		PrintComm.ID = "";
		PrintComm.MultID = "";
		//PrintComm.MthArr="web.DHCConsult:getConsultInfo&id:"+myid;
